...
std::vector<Variable> SATModel::createVariables(std::size_t count, const std::string& name_prefix) {
...
std::vector<Variable> SATModel::addWeightTableClauses(
	...
	assert(input_width == inputs.size());
	assert(output_width == outputs.size());
	...
	for (const auto& row : table_template) {
...
void SATModel::addXorClause(const std::vector<Variable>& values, Variable result) {
...
void SATModel::addAndClause(const std::vector<Variable>& values, Variable result) {
	std::vector<Literal> clause;
...
void SATModel::addOrClause(const std::vector<Variable>& values, Variable result) {
	std::vector<Literal> clause;
	for (auto value : values) {
		clause.push_back(-value);
		clause.push_back(result);
...
void SATModel::addSequentialCounterLessEqualClause(std::vector<Variable> x, int k) {
	auto n = x.size();
...
void SATModel::printLiteral(std::ostream& os, Literal lit, bool print_name) const {
